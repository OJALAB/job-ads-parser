from __future__ import annotations

import json
import sys
from pathlib import Path


def load_metrics(path: Path) -> dict:
    return json.loads(path.read_text(encoding="utf-8"))


def format_row(label: str, metrics: dict) -> str:
    mismatch_count = len(metrics.get("mapping_mismatches", []))
    return (
        f"{label:<34} "
        f"{metrics.get('mention_f1', 0.0):>6.3f} "
        f"{metrics.get('mapping_top1_accuracy', 0.0):>6.3f} "
        f"{metrics.get('mapping_topk_recall', 0.0):>6.3f} "
        f"{metrics.get('exact_mention_match_rate', 0.0):>6.3f} "
        f"{metrics.get('exact_top1_uri_match_rate', 0.0):>6.3f} "
        f"{mismatch_count:>4}"
    )


def main(argv: list[str]) -> int:
    if len(argv) < 3 or len(argv[1:]) % 2 != 0:
        print("Usage: python compare_model_benchmarks.py label1 metrics1.json [label2 metrics2.json ...]", file=sys.stderr)
        return 1

    print("model/case                         mF1   top1   topk exactM exactU mism")
    print("-" * 76)

    args = argv[1:]
    for index in range(0, len(args), 2):
        label = args[index]
        metrics = load_metrics(Path(args[index + 1]))
        print(format_row(label, metrics))
    return 0


if __name__ == "__main__":
    raise SystemExit(main(sys.argv))
